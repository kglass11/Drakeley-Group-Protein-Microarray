...
workdir <- "I:/Drakeley Group/PROTEIN MICROARRAYS/Experiments/120718 Monkey_IVTT"
...
for(i in 1:length(slides_list)) { 
  ...
  slides_all.df <- rbind.fill(slides_all.df,slides_list[[i]])
}
remove(i)
...
for(i in 1:nrow(slides_all.df)){
  ...
  temp <- which(samples.df$slide_no == slide & (samples.df$block_rep_1 == block))
...
annotation_targets.df <- filter(slides_all.df, slide_no==1, Block == 1)
annotation_targets.df <- annotation_targets.df[, c(1:3,5)]
...
for(i in 1:length(samples)){
  ...
  fore.df<-cbind.fill(fore.df,ite_out)
  colnames(fore.df)[length(colnames(fore.df))]<-samples_unique[i]
}

colnames(fore.df) <- gsub("X", "", colnames(fore.df))
rownames(fore.df) <- c(annotation_targets.df$target_id_unique)

#Background
back.df <- annotation_targets.df
for(i in 1:length(samples)){
  ite_out<-slides_all.df$B594.Median[which(slides_all.df$Sample==samples_unique[i])]
  back.df<-cbind.fill(back.df,ite_out)
  colnames(back.df)[length(colnames(back.df))]<-samples_unique[i]
}

colnames(back.df) <- gsub("X", "", colnames(back.df))
rownames(back.df) <- c(annotation_targets.df$target_id_unique)
...
targets_blank = c(grep("PBST wash", annotation_targets.df$ID, ignore.case = TRUE))
targets_buffer = c(grep("No DNA", annotation_targets.df$ID, ignore.case = TRUE))
targets_ref = c(grep("Fiducial", annotation_targets.df$ID, ignore.case = TRUE))
targets_std = c(grep("Ig", annotation_targets.df$ID, ignore.case = TRUE))
...
cor_buffer_mean <- mean(cor.5.matrix[targets_buffer,], na.rm = TRUE)
#SD median corrected MFI for all data points
cor_buffer_sd <- sd(cor.5.matrix[targets_buffer,], na.rm = TRUE)
#Cut-off for deviation from mean
...
cor_buffer_sample_mean <- colMeans(cor.5.matrix[targets_buffer,], na.rm = TRUE)
#Vectors to identify normal and deviant samples
...
cor_buffer_cov_normal <- sd(cor.5.matrix[targets_buffer, cor_normal], na.rm = TRUE)/mean(cor.5.matrix[targets_buffer, cor_normal], na.rm = TRUE)
...
for(i in 1:ncol(cor.5.matrix))
{
  temp <- sd(cor.5.matrix[targets_buffer,i], na.rm = TRUE)/mean(cor.5.matrix[targets_buffer,i], na.rm = TRUE)
...
cor_target_mean <- rowMeans(cor.5.matrix)
...
for(i in 1:nrow(cor_sample_deviant)){
  samples.df$exclude[which(samples.df$sample_id == cor_sample_deviant$sample_id[i])] <- "yes"
}
...
cor_target_mean_b1b2 <- cor_target_mean_b1
...
annotation_target_b1b2 <- annotation_target_b1
...
remove(cor_target_mean, cor_target_mean_b1,cor_target_mean_b1b2, annotation_target_b1, annotation_target_b2, annotation_target_b1b2)
...
samples_exclude <- samples.df$sample_id_unique[which(samples.df$exclude =="yes")]
...
#Sample metadata file, with samples removed if exclude == yes. 
  ...
  sample_meta_f.df <- samples.df[(!(samples.df$exclude == "yes") | is.na(samples.df$exclude)),]
...
#Normalized log data with samples as rows and targets as columns for every target(including controls)
  ...
  trans.norm2.matrix <- trans.norm.matrix[,]
...
#Prepare final data with GST subtracted, control targets removed - For Negs set to 0 ONLY! (norm4.matrix)
  ...
  samples_test <- samples.df$sample_id_unique[which(samples.df$sample_type =="test")]
  samples_control <- samples.df$sample_id_unique[which(samples.df$sample_type =="control")]
  ...
  #row.names(norm_sub3.df) <- norm_sub3.df$ID
  norm_sub4.df <- norm_sub3.df[,1:ncol(norm_sub.matrix)]
  rownames(norm_sub4.df) <- sub("_[^_]+$", "", rownames(norm_sub4.df))
  colnames(norm_sub4.df) <- sub("_[^_]+$", "", colnames(norm_sub4.df))
